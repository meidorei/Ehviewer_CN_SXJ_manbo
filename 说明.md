...
## 36. 漫画整理 Skill 稳定顺序调整

2026-08-12 已调整仓库内 `organize-ehviewer-skill-mega` 的默认系列排序策略，应用构建身份保持 `com.ehviewer.manbo.debug / 2.0.2.25 / 150` 不变喵

修改内容与影响范围：

- 每个系列继续以整理前 `TIME DESC` 列表中最靠前的成员作为系列块锚点，不按系列标题、作者或语义编号重排不同系列；独立漫画之间的原相对顺序保持不变喵
- 系列成员的 `itemOrder` 仍表示从旧到新的语义编号，但输出到 EhViewer 下载列表时改为降序，使最新一本位于系列块开头；相同编号或未知编号继续保持整理前的相对顺序喵
- 校对页明确显示“保留原列表锚点、系列内最新到最旧”，恢复页面顺序会回到新生成的稳定顺序；JSON `formatVersion: 2`、字段、人工拖动导出和安全写回契约均未改变喵
- 新增纯 Python 回归测试，覆盖交错系列聚拢、锚点顺序、最新到最旧、独立漫画稳定性、相同与未知编号稳定性，以及缺失或重复 GID 拒绝逻辑喵

验证结果与限制：

- 本次只修改 Skill、测试和说明文档，不修改 Android 应用代码、资源、数据库结构或版本，因此不需要构建 APK 喵
- 新增的 3 项 `unittest` 回归测试全部通过；使用现有安全目录中的 3197 项元数据前向验证时，2606 个系列块锚点顺序正确，352 个多本系列全部连续且成员降序零违规，独立漫画相对顺序保持不变喵
- `validate_order.py` 严格校验通过，校对页静态包含全部 3197 个 GID 和标题，页面恢复操作能够回到新生成顺序，`git diff --check` 通过喵
- Skill Creator 的 `quick_validate.py` 已调用，但本机现有 Python 均缺少其 `PyYAML` 依赖；未擅自联网安装依赖，已按该脚本源码执行等价的 frontmatter 必填字段、允许字段、名称格式和长度检查并通过喵
- 当前手机书库不会自动应用新策略，本次未执行 ADB、未连接或操作手机，也未重新整理现有数据库；新规则只对以后重新运行该 Skill 生成的顺序生效喵

## 37. 漫画整理 Skill 时间戳安全排序

2026-08-20 已修复 `organize-ehviewer-skill-mega` 写回排序时生成超大未来 `TIME` 的问题，Skill JSON 格式版本继续为 `2`，Android 应用版本保持 `com.ehviewer.manbo.debug / 2.0.2.25 / 150` 不变喵

修改内容与影响范围：

- `apply_order.py` 现将确认后的 GID 顺序映射到源数据库已有的、按降序排列的唯一 `TIME` 槽位，只重排这些真实时间值，不再生成合成时间戳，也不改变时间范围喵
- 源库若存在重复 `TIME`，脚本会在创建备份、排序副本或事务写入前停止，避免为了强制唯一排序而篡改真实下载时间喵
- 排序完成后除既有完整性、GID 顺序、唯一性及非 `TIME` 字段校验外，还会确认写回前后的 `TIME` 多重集合完全一致，并在报告中记录 `permute-existing-unique-times` 策略与时间值保留状态喵
- 快照读取增加 `GID DESC` 作为相同时间戳下的确定性次排序；Skill 说明与详细流程同步明确时间戳唯一性前置条件喵

验证结果与限制：

- 新增 `test_apply_order.py`，覆盖真实时间槽位置换、源库重复时间戳拒绝、确认顺序缺失或重复 GID 拒绝，以及真实 SQLite 副本写回后 `TIME` 集合与非时间字段保留喵
- 本次只修改 Skill 脚本、测试和说明文档，不修改 Android 应用代码、资源、数据库结构或版本，因此不构建 APK、不执行 ADB，也不操作手机数据库喵
- 两个测试模块共 7 项 `unittest`、全部技能脚本语法检查、`apply_order.py --help` 与 `git diff --check` 均通过喵
- Skill Creator 的 `quick_validate.py` 已调用，但本机隔离 Python 缺少其 `PyYAML` 依赖；未擅自安装依赖，已按其源码对现有简单 frontmatter、名称、描述和 TODO 占位规则做等价检查喵
- 已知限制是源库若历史上存在重复 `TIME`，该 Skill 会安全停止而不自动修复；需要先由用户确认是否允许另行设计最小扰动或应用内重排方案喵

## 38. 漫画整理 Skill 全量跨语言发现轮

2026-08-21 已为 `organize-ehviewer-skill-mega` 增加全量跨语言发现轮，Skill 的最终排序 JSON 继续使用 `formatVersion: 2`，新增发现批次与发现结果使用独立 `formatVersion: 1`，Android 应用版本保持 `com.ehviewer.manbo.debug / 2.0.2.25 / 150` 不变喵

修改内容与影响范围：

- 新增默认每批 75 本的安全发现批次生成器；3184 本目录会生成 43 批，每批只包含 GID、原位置、原始标题和日文标题，不向模型暴露数据库、文件路径、Cookie 或设备信息喵
- 新增发现结果严格校验器，要求全部 GID 恰好一条紧凑标注，并限制作者别名不超过 3 个、作品别名不超过 5 个、语义作品键不超过 96 字符及置信度为 `0–1` 喵
- 候选生成现可利用跨语言作品别名和语义作品键，并输出低置信大碰撞审核组与未解决单本索引；这些字段仅提高候选召回，不自动确认系列合并喵
- 新增运行清单、注册工具和清理工具；只有前后两次通过且哈希未变的验证报告已登记，并且用户明确确认后，才会删除已登记的临时发现和审核文件，最终 JSON、报告、快照与备份始终保留喵
- 工作流由三轮语义审核扩展为候选、作者桶、未解决索引和全局冲突四轮审核，全球索引同时覆盖多本系列、单本摘要和大碰撞审核组喵

验证结果与限制：

- 新增 4 项发现工作流测试，覆盖 3184 本生成 43 个完整批次、发现结果缺失或指纹错误拒绝、日中英罗马音候选召回、同作者不同作品不合并、大碰撞限制和二次验证后清理保留策略喵
- 全部 Python 测试当前共 11 项且均通过，全部技能脚本语法检查通过；本次仅修改 Skill、测试和说明文档，不构建 APK、不执行 ADB，也不操作手机数据库喵
- 均衡模式在现有标题长度样本上的估算为约 43 次发现请求、每次约 `6–10k` tokens、完整发现与后续审核约 `35–60` 万 tokens；实际用量会随标题语言、别名数量和低置信冲突数量变化喵
- 已知限制是跨语言发现仍是候选召回而非事实证明，通用语义作品键超过 40 条时会安全降级为人工低置信审核组，不会自动连接为单一系列喵

## 39. 漫画整理 Skill 当前时间顺序写回

2026-08-22 已将 `organize-ehviewer-skill-mega` 的排序写回时间策略改为当前时间递减秒序列，Skill 最终排序 JSON 继续为 `formatVersion: 2`，Android 应用版本保持 `com.ehviewer.manbo.debug / 2.0.2.25 / 150` 不变喵

修改内容与影响范围：

- `apply_order.py` 在备份核验完成后取得当前毫秒时间，把确认顺序的第一本写为该时间，其后每本严格提前一秒；例如 3184 本会形成约 53 分钟的连续排序区间喵
- 后续应用下载使用更晚的实时毫秒时间，因此会自然显示在这个整理区间之前，不会被此次排序区间压在下方喵
- 源库原有 `TIME` 是否重复不再阻止写回；排序副本仍要求最终 `TIME` 全部唯一、GID 顺序精确、数据库完整且所有非 `TIME` 字段摘要不变喵
- 报告新增 `current-time-descending-seconds`、写入基准毫秒和每步 `1000` 毫秒，便于核验本次策略喵

验证结果与限制：

- 已更新纯 Python 回归测试，覆盖当前时间起点、每秒递减、源库时间戳重复仍可排序、确认 GID 重复拒绝，以及 SQLite 写回后的非 `TIME` 字段保留喵
- 本次只修改 Skill、测试和说明文档，不构建 APK、不执行 ADB，也不操作手机数据库喵
- 已知限制是此次排序会把所有已整理漫画的下载时间显示为写回时刻之前的连续秒序列，不再保留每一本原先的历史下载日期喵

## 40. 漫画整理 Skill 历史快照候选复用

2026-08-22 已为 `organize-ehviewer-skill-mega` 增加仅供审阅的历史快照候选复用脚本，最终排序 JSON 仍为 `formatVersion: 2`，Android 应用版本保持 `com.ehviewer.manbo.debug / 2.0.2.25 / 150` 不变喵

修改内容与影响范围：

- 新增 `seed_historical_decisions.py`，仅当当前目录与明确指定的历史目录中同一 GID 的标题及日文标题完全一致时，才复用历史语义字段；标题有变化或新出现的 GID 一律先保持独立喵
- 脚本中列出的高召回候选覆盖明确的同名、译本、续篇或分卷线索，但统一降为低置信度并保留人工校对要求，不会直接写入数据库或绕过离线审核页喵
- 输出记录复用、新增、标题变化和人工审阅候选组数量，且拒绝覆盖已有输出文件，便于追溯候选来源喵

验证结果与限制：

- 本次运行将针对当前 USB 导出的 3191 本快照生成审阅草案；手机数据库仍只读导出，尚未执行排序写回、替换或清理中间文件喵
- 已知限制是历史结论只用于标题未变条目的候选种子，不等于当前最终确认；用户仍须在离线页面检查、导出 JSON 并明确确认后，后续安全写回流程才能继续喵

## 41. 项目级 Codex Skill 忽略规则

2026-08-24 已将项目级 Skill 目录 .codex/skills/organize-ehviewer-skill-mega 纳入 Git 忽略，Android 应用版本保持 com.ehviewer.manbo.debug / 2.0.2.25 / 150 不变喵

修改内容与影响范围：

- .gitignore 新增 /.codex/，因此项目内 Codex Skill 与其配套文件不会进入 Git 提交喵
- 本次仅调整本仓库的 Codex 配置与 Git 忽略规则，不修改 Android 应用代码、资源、构建配置或用户数据喵

验证结果与限制：

- 已通过 git check-ignore 确认 .codex/skills/organize-ehviewer-skill-mega/SKILL.md 命中 /.codex/ 规则，且 git diff --check 通过喵
- 被忽略的项目级 Skill 不会随 Git 克隆或提交共享给其他开发者；如需团队共享，应移除该忽略规则并提交完整 Skill 目录喵