...
- **书签更新**：书签栏可以检查全部书签或长按检查单个书签，并连续翻页保存 checkpoint 之后的全部更新 GID；成功打开一个书签后，其他书签中相同 GID 的未读内容也会联动扣减，角标超过 20 条时仍显示 `20+`。
...
- **首页手动分割线**：首页可从悬浮按钮菜单把“上次看到这里”重置到当前已加载列表顶部；打开、刷新、翻页和切换账号或站点都不会自动移动它。
...
当前 v141 APK 大小为 `32591879` 字节，SHA-256 为 `F09440A7A74C187FFCFDF82381B4919A3235C84E5665D5B15E721B66565C108E`，`output-metadata.json` 确认变体为 `appReleaseDebug`。
...
逐书签检查、全局扫描降级和长按单书签检查会连续翻页到旧 checkpoint 或列表末尾，并保存期间发现的全部书签更新 GID。书签未读关系不再限制为 21 条，真实数量超过 20 时角标仍统一显示 `20+`；追更栏继续保留原有 21 条明细策略。旧版本已经裁掉的 GID 无法恢复，服务器后来删除的未读漫画会在对应书签下一次成功打开时随进入前的未读批次一起清除。
