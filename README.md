...
- **上次更新到这**：列表中以青绿色加粗分割线记录上一次成功打开列表时的顶部边界；其他书签因重复漫画联动清零时，会改用该书签最近一次成功更新记录的第一项边界，延迟查看期间出现的新漫画仍保留在分割线上方。
- **首页手动分割线**：首页可点击置顶按钮或长按分割线，经确认后把“上次看到这里”重置到当前已加载列表顶部；打开、刷新、翻页和切换账号或站点都不会自动移动它。
- **可控制的后台检查**：更新任务显示当前方式、来源、进度、扫描页数和完整更新时间，并支持暂停、继续和停止。
- **更新资源优化**：列表解析不再为每页创建无用线程池；连续书签扫描只保留新增 GID，并复用预编译语句原子写入，减少长任务的线程、内存和数据库压力。
